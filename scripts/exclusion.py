...
def add():
    ...
    additions = outer[(outer['Requestor_x'].isnull()) &
                      outer['Requestor_y'].notnull()]
    ...
    duplicates = outer[(outer['Requestor_x'].notnull()) &
                       (outer['Requestor_y'].notnull())]

    # duplicates changing owner
    new_owner = duplicates[duplicates['Requestor_x']
                           != duplicates['Requestor_y']]
    new_owner_clean = new_owner.drop(columns=['Requestor_x', 'Date Added_x'])
    new_owner_clean.rename(
        columns={'Requestor_y': 'Requestor', 'Date Added_y': 'Date Added'}, inplace=True)

    # requests with same owner - disrearded
    same_owner = duplicates[duplicates['Requestor_x']
                            == duplicates['Requestor_y']]

    # old list minus - duplicates
    keeps = outer[(outer['Requestor_y'].isnull()) | (
        outer['Requestor_x'] == outer['Requestor_y'])]
    ...
    new_list['Date Added'] = new_list['Date Added'].apply(
    ...
    # DUPLICATED REMOVED
    duplic_list = duplicates[duplicates['Requestor_x']
                             != duplicates['Requestor_y']]
    duplic_list['Requestor_y'] = 'moved to ' + \
        duplic_list['Requestor_y'] + ' list'
    duplic_list_clean = duplic_list.iloc[:, [4, 0, 1, 2, 3]]
    duplic_list_clean.rename(columns={'Date Added_y': 'Date Removed', 'Requestor_x': 'Requestor',
                             'Date Added_x': 'Date Added', 'Requestor_y': 'Comments'}, inplace=True)
    duplic_list_clean['Date Removed'] = pd.to_datetime(
        duplic_list_clean['Date Removed']).dt.date
    duplic_list_clean['Date Added'] = pd.to_datetime(
        duplic_list_clean['Date Added']).dt.date
    ...
    duplic_list_clean.to_excel(
        writer, sheet_name='Duplicates removed', index=False)
    writer.save()
